...
from SCcfg_graph_V14_25_config import get_config, get_device_config, get_dataset, get_model_optim_schedu_criter
...
# 如果使用现有的数据集，则调用，否则重新划分数据集
if config["previous_dataset"]:
	# 读取之前划分的数据集
	dataset_train_valid_test = pickle.load(open(config["previous_dataset_path"], 'rb'))
	# Final_index_list是Phase4中的Multimodal_Fusion的数据集中数据样本的index list
	# 使用Final_index_list的目的在于检查在config中调用的数据集(Phase4中的Multimodal_Fusion的数据集)与当前我希望使用的数据集是否一致
	Final_index_list = dataset.Final_index_list
	# 读取之前划分的数据集dataset_train_valid_test保存的index_list和indices_list
	...
	if set(Final_index_list) != set(trainset_item_index_list + validset_item_index_list + testset_item_index_list):
		...
		# 如果一致，则按照trainset_item_index_list的index找出dataset对应的data的indice，对testset_item_index_list同样操作
		trainset_indices_list = []
		validset_indices_list = []
		testset_indices_list = []
		print("按照trainset_item_index_list的index找出dataset对应的data的indice，对testset_item_index_list同样操作")
	...
	# 分配trainset_indices_list、testset_indices_list、validset_indices_list
	...
	# trainset_item_index_list, validset_item_index_list 和 testset_item_index_list 记录训练集和测试集中数据的index(指的是data中的"index"字段中的index)
	...
	# trainset_indices_list, validset_indices_list 和 testset_indices_list 记录训练集和测试集中数据的indices(指的是train_dataset和test_dataset中的indices)
...
# 保存上述6个数据：trainset_item_index_list, testset_item_index_list, trainset_indices_list, testset_indices_list
...
# logger.info("{:<25}: {}".format("loss function parameter", criterion.get_param()))
logger.info("")
# log记录Model structure
	...
	with torch.no_grad():
		with tqdm(total=len(dataset), leave=False) as p_bar:
			for i, data in enumerate(dataset):
				...
				loss = criterion(pred_float, target) + d_loss * config["wight_pen"]  # Focal loss  # 惩罚权重可以根据需要调整
				...
				allocated_memory = torch.cuda.memory_allocated() / (1024 * 1024)  # 转换为 MB 单位
				p_bar.set_postfix({'allocated GPU memory': '%.4fMB' % allocated_memory})  # 实时更新进度条后面的说明
...
for epoch in range(start_epoch, num_epochs):
	...
	with tqdm(total=len(train_dataset), leave=False) as p_bar:
		for i, data in enumerate(train_dataset):
			...
			target = data["label"][config["vul"]].to(device_cuda[0])  # shape=(bs,), type: torch.int64
			loss = criterion(pred_float, target) + d_loss * config["wight_pen"]  # Focal loss
			loss.backward()
			optimizer.step()
	...
	# 检查saved_model_folder中是否只有最新的config["max_saved_model"]个epoch的模型，然后把其他的删除
	epoch_list = []
